...
export default function Adminlogin() {
    ...
    const submitHandler = (e) => {
        e.preventDefault();
        setloading(true);
        axios.post('https://e-commerce-b-qpu8.onrender.com/api/auth/admin', login).then(res => { alert(res.data); setloading(false); navigate('/admin') }).catch(err => {alert(err.response.data); setloading(false);});
    }
    if (loading) {
        return <Loading />
    }
