...
export default function Placeorder() {
    ...
    const placeorder = () => {
        axios.post('https://e-commerce-b-qpu8.onrender.com/api/order', { shipping: shippings, payment: payment, items: cart._id, summery: count * cart.price }, { headers: { 'x-token': `${localStorage.getItem('token')}` } }).then(res => { setorderdata(res.data); navigate('/order') });
    }
    return (
