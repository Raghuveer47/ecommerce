...
export default function Profile() {
    ...
    const submitHandler = (e) => {
        e.preventDefault();
        axios.put('https://e-commerce-b-qpu8.onrender.com/api/user/', profile, { headers: { 'x-token': `${localStorage.getItem('token')}` } }).then(res => { setprofile(res.data); alert("profile updated sucssfully") });
    }
    const changeHandler = (e) => {
        setprofile({ ...profile, [e.target.name]: e.target.value })
    }
    useEffect(() => {
        axios.get('https://e-commerce-b-qpu8.onrender.com/api/user/', { headers: { 'x-token': `${localStorage.getItem('token')}` } }).then(res => setprofile(res.data));
    }, []);
    return (
