...
export default function Login() {
    ...
    const submitHandler = (e) => {
        e.preventDefault();
        setloading(true);
        axios.post('https://e-commerce-b-qpu8.onrender.com/api/auth/login', login).then(res => { localStorage.setItem('token', res.data.token); setloading(false); navigate('/') }).catch(err => alert(err.response.data));
    }
    if (loading) {
        return <Loading />
    }
