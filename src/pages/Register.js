...
export default function Register() {
    ...
    const submitHandler = (e) => {
        e.preventDefault();
        setloading(true);
        axios.post(`https://e-commerce-b-qpu8.onrender.com/api/auth/register`, register).then(res => { alert(res.data); setloading(false); navigate('/login') }).catch(err => alert(err.response.data));
    }
    if (loading) {
        return <Loading />
    }
